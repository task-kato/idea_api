    ...
    describe 'アイデア登録' do
      ...
      context '正常系' do
        example 'リクエストのcategory_nameがcategoriesテーブルのnameに存在する' do
          # 登録されているカテゴリー名をリクエスト
          expect { post api_ideas_url, params: param_hash.merge!(category_name: category.name) }
      ...
      context '準正常系' do
        example 'リクエストのbodyのみがnil' do
          # 登録されているカテゴリー名をリクエスト
          expect { post api_ideas_url, params: invalid_param_hash.merge!(category_name: category.name) }
